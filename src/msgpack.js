function serializeMsgPack(data) {
	...
	function append(data) {
		switch (typeof data) {
			...
			case 'string':
				...
				break;
			case 'object':
				if (Array.isArray(data)) appendArray(data);
				else if (data instanceof Date) appendDate(data);
				else if (data === null) appendNull(data);
				else if (data.length) {
					appendBinary(data);
				} else {
					appendObject(data);
	...
	function appendBinary(data) {
		...
		if (length <= 0xff) appendBytes([ 0xc7, length ]);
		else if (length <= 0xffff) appendBytes([ 0xc8, length >>> 8, length ]);
		else appendBytes([ 0xc9, length >>> 24, length >>> 16, length >>> 8, length ]);
...
function deserializeMsgPack(array) {
	...
	function read() {
		...
		if (byte === 0xc1) throw new Error('Matched no byte code. Value is 0xc1 error'); // never used
